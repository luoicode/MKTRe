...
import { useSlots, fmtVndDong, fmtInt } from "@/lib/reports";
...
type SlotReportRow = Tables<"slot_reports">;
type SlotReportMemberRow = { profile: ProfileRow; report: SlotReportRow | null };
...
function LeaderSlotReport() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (!teamIds.length) {
        return { rows: [] as SlotReportMemberRow[], missing: [] as SlotReportMemberRow[] };
      }
  ...
  const slotName = useMemo(
    () => slots?.find((s) => s.id === slotId)?.slot_name ?? "",
    [slots, slotId],
  );
              ...
              <thead className="bg-slate-100 text-left">
                <tr>
                  {[
                    ...
                    <th key={h} className="border-b px-2 py-2 font-semibold">
                      {h}
                    </th>
                  ))}
                </tr>
                ...
                {data?.rows.map(({ profile: p, report: r }) => (
                  <tr key={p.id} className={!r ? "bg-amber-50" : ""}>
                    ...
                    <td className="border-b px-2 py-1.5">
                      {!r ? (
                        <Badge variant="destructive">Chưa báo cáo {slotName}</Badge>
                      ) : (
                        <Badge
                          variant={
                    ...
                    <td className="border-b px-2 py-1.5">
                      {r ? fmtVndDong(Number(r.ads_cost)) : "—"}
                    </td>
                    <td className="border-b px-2 py-1.5">
                      {r ? fmtInt(Number(r.mess_count)) : "—"}
                    </td>
                    <td className="border-b px-2 py-1.5">
                      {r ? fmtInt(Number(r.data_count)) : "—"}
                    </td>
                    <td className="border-b px-2 py-1.5">
                      {r ? fmtInt(Number(r.closed_orders)) : "—"}
                    </td>
                    <td className="border-b px-2 py-1.5">
                      {r ? fmtVndDong(Number(r.daily_data_revenue)) : "—"}
                    </td>
                    <td className="border-b px-2 py-1.5">
                      {r ? fmtInt(Number(r.total_orders)) : "—"}
                    </td>
                    <td className="border-b px-2 py-1.5">
                      {r ? fmtVndDong(Number(r.total_revenue)) : "—"}
                    </td>
