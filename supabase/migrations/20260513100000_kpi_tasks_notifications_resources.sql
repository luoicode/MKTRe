-- KPI write policies plus task/checklist/notification/resource foundations.
...
CREATE POLICY "kpi_leader_manager_insert" ON public.kpi_targets
  ...
  WITH CHECK (
    ...
    OR (
      ...
      AND (user_id IS NULL OR public.can_view_user(user_id))
    )
  );
...
CREATE POLICY "kpi_leader_manager_delete" ON public.kpi_targets
...
CREATE POLICY "tasks_leader_manager_write" ON public.tasks
  ...
  WITH CHECK (
    team_id IS NOT NULL
    AND public.can_manage_team_kpi(team_id)
    AND public.can_view_user(assigned_to)
  );
