ALTER TABLE public.floating_leads
  ...
  ADD COLUMN IF NOT EXISTS blocked_sale_ids uuid[] NOT NULL DEFAULT '{}'::uuid[],
  ADD COLUMN IF NOT EXISTS last_claimed_at timestamptz,
  ADD COLUMN IF NOT EXISTS closed_by uuid REFERENCES public.profiles(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS closed_at timestamptz,
  ADD COLUMN IF NOT EXISTS is_closed boolean NOT NULL DEFAULT false;

CREATE INDEX IF NOT EXISTS idx_floating_leads_is_closed
  ON public.floating_leads(is_closed);

CREATE INDEX IF NOT EXISTS idx_floating_leads_closed_by
  ON public.floating_leads(closed_by);
...
BEGIN
  ...
  SET
    ...
    blocked_sale_ids = CASE
      WHEN p_sale_id = ANY(blocked_sale_ids) THEN blocked_sale_ids
      ELSE array_append(blocked_sale_ids, p_sale_id)
    END,
    updated_at = now()
  WHERE assigned_sale_id = p_sale_id
    AND is_closed = false
